...
// === REFERRAL: record purchase ===
if (receipt.status === 1) {
  const buyerWallet = account;
  const txHash = tx.hash;
  const usdt = usdtAmount;
  const tokens = calculateTokens();
  ...
  try {
    ...
    console.error("[Referral] recordPurchase failed", e);
  }
}
      if (receipt.status === 1) {
    ...
    <div className="min-h-screen" style={{ background: '#E8E4F3' }}>
      ...
      <header className="border-b bg-white/60 backdrop-blur-md sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <img src="/logo_wht.svg" alt="Expoin" className="h-8" />
      ...
      <main className="container mx-auto px-4 py-12">
        ...
        <footer className="mt-16 text-center text-gray-600 pb-8">
          <p className="text-sm">
            © 2026 Expoin. All rights reserved. |{' '}
